...
def step2_umbrella():
    options = {"да": True, "нет": False}
    print(
        f"Выберите, пойдёт ли сегодня дождь? {list(options.keys())[0]}/"
        f"{list(options.keys())[1]}"
    )
    answer = input()
    while answer.lower() not in options.keys():
        print(
            "Неверный ответ. Попробуйте что-то из предложенных: "
    ...
    if options[answer]:
        print(
            "Уточка-маляр 🦆 счастлива, не осталась без зонта ☂ в плохую " 
            "погоду!",
            "Но уточка устала и решила вернуться домой спать",
        )
    else:
        print(f"Зонтик ☂ не понадобился, уточка-маляр 🦆 отправилась в бар.")
        drinks = {"А": "кофе", "Б": "сидр", "В": "напиток, известный "
                                                 "только уткам"}
        print("Что же выпить уточке?")
...
def step2_no_umbrella():
    ...
    if options[answer]:
        ...
        options = {
            "А": "вернуться домой и отдыхать",
            "Б": "бежать в бар",
            "В": "погрустить в парке на лавочке",
        }
        ...
        if options[answer] == "вернуться домой и отдыхать":
            print(
                "Уточка 🦆 расстроилась из-за дождя, устала и пошла "
                "отдыхать домой."
            )
        elif options[answer] == "бежать в бар":
            print(
                "Уточка 🦆 влетела в бар, испугав барменов. Уточка 🦆 не "
                "ознакомилась с меню, но бармены так напуганы, что приготвят "
                "любой напиток, что вы закажете."
            )
            ...
            print(":(")
            print(
                "Но тут уточка 🦆 увидела другую уточку 🦆, у которой был "
                "зонтик. +1 друг, ура!"
            )

    else:
        print(
            "Уточке 🦆 не взяла зонтик ☂, но дождя не было - "
            "вот это удача! Надо это отпраздновать!"
        )
